"use strict";(()=>{function T(f,t,e=[]){let s=0,i=0,n=0,o=100,u;do{u=!1,s=Math.random()*(window.innerWidth-f),i=Math.random()*(window.innerHeight-t);for(let r of e){let c=[Math.min(...r.targetPoints.map(a=>a[0])),Math.min(...r.targetPoints.map(a=>a[1])),Math.max(...r.targetPoints.map(a=>a[0])),Math.max(...r.targetPoints.map(a=>a[1]))];if(!(s+f<c[0]||s>c[2]||i+t<c[1]||i>c[3])){u=!0;break}}n++}while(u&&n<o);return[s,i]}function P(f){return f.map(t=>t.slice(0,2))}function E(f,t,e,s){return Math.hypot(e-f,s-t)}var C="0.0.1.1";function H(f){let t=document.createElement("div");t.id="controls",t.innerHTML=`
