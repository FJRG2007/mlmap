"use strict";(()=>{function C(u,e,t=[]){let s=0,i=0,n=0,o=100,y;do{y=!1,s=Math.random()*(window.innerWidth-u),i=Math.random()*(window.innerHeight-e);for(let c of t){let d=[Math.min(...c.targetPoints.map(l=>l[0])),Math.min(...c.targetPoints.map(l=>l[1])),Math.max(...c.targetPoints.map(l=>l[0])),Math.max(...c.targetPoints.map(l=>l[1]))];if(!(s+u<d[0]||s>d[2]||i+e<d[1]||i>d[3])){y=!0;break}}n++}while(y&&n<o);return[s,i]}function P(u){return u.map(e=>e.slice(0,2))}function D(u,e,t,s){return Math.hypot(t-u,s-e)}var X=(()=>{function u(c,d,l,p){if(l===d.length-1)return p(c);let h,m=d[l],b=Array(m);for(h=m-1;h>=0;--h)b[h]=u(c[h],d,l+1,p);return b}function e(c){let d=[];for(;typeof c=="object";)d.push(c.length),c=c[0];return d}function t(c){let d,l;return typeof c=="object"?(d=c[0],typeof d=="object"?(l=d[0],typeof l=="object"?e(c):[c.length,d.length]):[c.length]):[]}function s(c){let d,l=c.length,p=Array(l);for(d=l-1;d>=0;--d)p[d]=c[d];return p}function i(c){return typeof c!="object"?c:u(c,t(c),0,s)}function n(c,d){d=d||!1;let l,p,h,m,b,L,v,g,k,S=c.length,B=S-1,E=new Array(S);for(d||(c=i(c)),h=0;h<S;++h){for(v=h,L=c[h],k=y(L[h]),p=h+1;p<S;++p)m=y(c[p][h]),m>k&&(k=m,v=p);for(E[h]=v,v!=h&&(c[h]=c[v],c[v]=L,L=c[h]),b=L[h],l=h+1;l<S;++l)c[l][h]/=b;for(l=h+1;l<S;++l){for(g=c[l],p=h+1;p<B;++p)g[p]-=g[h]*L[p],++p,g[p]-=g[h]*L[p];p===B&&(g[p]-=g[h]*L[p])}}return{LU:c,P:E}}function o(c,d){let l,p,h,m,b,L=c.LU,v=L.length,g=i(d),k=c.P;for(l=v-1;l>=0;--l)g[l]=d[l];for(l=0;l<v;++l)for(h=k[l],k[l]!==l&&(b=g[l],g[l]=g[h],g[h]=b),m=L[l],p=0;p<l;++p)g[l]-=g[p]*m[p];for(l=v-1;l>=0;--l){for(m=L[l],p=l+1;p<v;++p)g[l]-=g[p]*m[p];g[l]/=m[l]}return g}let y=Math.abs;return function(c,d,l){return o(n(c,l),d)}})();function I(u){localStorage.setItem(`mlmap:workspace<${u.id}>`,JSON.stringify(u))}function N(u){let e=localStorage.getItem("mlmap:currentWorkspaceId");if(!e){e=Date.now().toString(),localStorage.setItem("mlmap:currentWorkspaceId",e);let s={id:e,name:"New Workspace",version:"1.0",layout:{}};return localStorage.setItem(`mlmap:workspace<${e}>`,JSON.stringify(s)),s}let t=localStorage.getItem(`mlmap:workspace<${u||e}>`);return t?JSON.parse(t):null}function $(u){localStorage.removeItem(`mlmap:workspace<${u}>`)}function Y(u){let e=`mlmap:workspace<${u}>`,t=JSON.parse(localStorage.getItem(e)||`{id:"${u}",name:"Reset Workspace",version:"1.0",layout:{}`);localStorage.setItem(e,JSON.stringify({id:u,name:t?.name,version:"1.0",layout:{}}))}function O(){let u=[];for(let e=0;e<localStorage.length;e++){let t=localStorage.key(e);if(t&&t.startsWith("mlmap:workspace<")){let s=localStorage.getItem(t);s&&u.push(JSON.parse(s))}}return u}var A="0.0.2.3",T=[];function K(u){let e=document.createElement("div");e.id="controls",e.innerHTML=`
