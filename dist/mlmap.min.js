"use strict";(()=>{function C(u,e,t=[]){let s=0,i=0,n=0,o=100,y;do{y=!1,s=Math.random()*(window.innerWidth-u),i=Math.random()*(window.innerHeight-e);for(let c of t){let d=[Math.min(...c.targetPoints.map(l=>l[0])),Math.min(...c.targetPoints.map(l=>l[1])),Math.max(...c.targetPoints.map(l=>l[0])),Math.max(...c.targetPoints.map(l=>l[1]))];if(!(s+u<d[0]||s>d[2]||i+e<d[1]||i>d[3])){y=!0;break}}n++}while(y&&n<o);return[s,i]}function P(u){return u.map(e=>e.slice(0,2))}function D(u,e,t,s){return Math.hypot(t-u,s-e)}function I(u){let e=`mlmap:workspace<${u.id}>`;localStorage.setItem(e,JSON.stringify(u))}function N(u){let e=localStorage.getItem("mlmap:currentWorkspaceId");if(!e){e=Date.now().toString(),localStorage.setItem("mlmap:currentWorkspaceId",e);let i={id:e,name:"New Workspace",version:"1.0",layout:{}};return localStorage.setItem(`mlmap:workspace<${e}>`,JSON.stringify(i)),i}let t=`mlmap:workspace<${u||e}>`,s=localStorage.getItem(t);return s?JSON.parse(s):null}function q(u){let e=`mlmap:workspace<${u}>`;localStorage.removeItem(e)}function K(u){let e=`mlmap:workspace<${u}>`,t=JSON.parse(localStorage.getItem(e)||`{id:"${u}",name:"Reset Workspace",version:"1.0",layout:{}`);localStorage.setItem(e,JSON.stringify({id:u,name:t?.name,version:"1.0",layout:{}}))}function $(){let u=[];for(let e=0;e<localStorage.length;e++){let t=localStorage.key(e);if(t&&t.startsWith("mlmap:workspace<")){let s=localStorage.getItem(t);s&&u.push(JSON.parse(s))}}return u}var O="0.0.2.1";function z(u){let e=document.createElement("div");e.id="controls",e.innerHTML=`
